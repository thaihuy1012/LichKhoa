...
## D-034 — SPEC v1.9: "Báo thức đúng ngày" — lời nhắc vào danh sách riêng + Tự động hóa 00:05 tạo báo thức Đồng hồ, milestone M8 (2026-09-19 · người quyết: Chủ dự án; thiết kế: Kiến trúc sư)
- Bối cảnh: báo thức Đồng hồ chỉ có giờ → IN-11 chặn "Thêm báo thức" ngoài 24 h (§8.10a). Chủ dự án mang ý tưởng: lời nhắc (đủ ngày + giờ) làm "kho hẹn", Tự động hóa mỗi sáng quét lời nhắc đến hạn hôm nay và tạo báo thức — không cần app chạy nền; đã chấp nhận 4 rủi ro (không đảm bảo chạy, chạy trùng, báo thức tồn trong Đồng hồ, cần danh sách riêng); chốt làm tính năng chính thức = nút thứ ba trong hộp nhắc.
- Kiến trúc sư quyết: (1) payload KHÔNG đổi — đúng `reminderText`; danh sách đích do Phím tắt thứ ba `ThemBaoThucNgay` ghim (bản sao `ThemLoiNhac` đổi List = `BaoThuc`). BÁC phương án thêm dòng "tên danh sách" vào payload: Cách B của M7 nhận biết ghi chú bằng "Item 3 has any value" nên thêm dòng sẽ phá hợp đồng, và List của "Add New Reminder" là đối tượng danh sách, không nhận chuỗi tin cậy. (2) Trường thứ ba `AppState.dayAlarmShortcutName` + action + ô nhập ở Xem trước, lặp đúng mẫu 2 trường cũ; prop bắt buộc như D-033; JSON vẫn version 1. (3) Hàm mới `dayAlarmWindow` → `past|today|too-early|ok`, KHÔNG sửa `reminderWindow` (test khóa T-7.1): chặn "hôm nay" (Tự động hóa hôm nay đã chạy → dùng Thêm báo thức) và giờ < 00:30 (`DAY_ALARM_MIN_TIME`; Tự động hóa 00:05 có thể trễ); không chặn theo 24 h.
- (4) Phía iPhone tách Phím tắt có tên `TaoBaoThucSang` (Find Reminders → Repeat with Each → If Due Date is after Current Date → Create Alarm → Edit Reminder: Set Is Completed), Tự động hóa chỉ "Run Shortcut" nó: chạy tay thử được ngay, là đường dự phòng khi máy tắt; đánh dấu hoàn thành chống trùng và để thông báo Reminders làm lưới dự phòng khi Tự động hóa không chạy. (5) Chữ hướng dẫn: chú thích tĩnh dưới mỗi nút + dòng trạng thái 5 trạng thái; không cảnh báo độ tin cậy trong hộp (tiếng ồn hằng ngày) — ghi HUONG-DAN §E.3 + thẻ Hướng dẫn + §8.11.
- Đã kiểm chứng (WebSearch/WebFetch 2026-09-19): iOS 17 Tự động hóa có "Run Immediately" / "Notify When Run"; "Edit Reminder" đặt được Is Completed / Due Date; "Create Alarm" luôn tạo mới, không sửa cái cũ; "Find Reminders" lọc + sort theo Due Date. CHƯA kiểm được bằng lệnh: Due Date của lời nhắc tạo bằng "Add New Reminder" có mang giờ → thử tay G1/G2; hỏng thì thêm "Edit Reminder: Set Due Date" vào `ThemBaoThucNgay` (tài liệu, không sửa mã).
- Phiếu đề xuất M8 (làn Gemini NHIỀU): T-8.1 lõi thuần + state, test mới `tests/unit/dayalarm.test.ts` (gemini) → T-8.2 nút thứ ba + chú thích + trạng thái + ô nhập + i18n + CSS, 8 file (gemini; phiếu phải ghi đúng `src/core/i18n/vi.json`, `en.json`, `src/ui/styles.css` — bài học T-7.2) ∥ T-8.3 HUONG-DAN §E.3 + thẻ Hướng dẫn (gemini) → T-8.END E2E `m8-dayalarm.spec.ts` với `page.clock.setFixedTime` (kiem-thu / gemini) → thử tay G1–G5 SAU thử tay M7 A–F.

## D-035 — SPEC v1.10: Việc có giờ hạn (`Todo.dueTime`) + sửa hạn sau khi tạo, milestone M9; kèm phán quyết duyệt M8 (2026-09-19 · người quyết: Chủ dự án; thiết kế: Kiến trúc sư)
- Bối cảnh: B-011 — Chủ dự án muốn hạn việc = ngày + giờ, sửa được sau khi tạo (bỏ "nhắc 2 lần"). `Todo.due` (ISODate) xuất hiện 43 chỗ trong 13 file test khóa và mọi so sánh ngày (`week.ts:78`, `common.ts:167`, `store.ts:16`).
- Hai phương án: (A) đổi kiểu `due` thành `YYYY-MM-DDTHH:mm` — mọi so sánh `===`/`<` theo ngày sai, phải migrate sao lưu và sửa test khóa; (B) THÊM `dueTime?: 'HH:mm'` cạnh `due`, không đổi kiểu, `normalizeState` lọc.
- Quyết định: (B). Cùng ngày sắp theo giờ (`cmpTodo`), `sameTodoGroup` thêm cùng giờ; nhãn "Hôm nay 14:00" / "6/10 14:00", **"Quá hạn" theo NGÀY** không kèm giờ (Chủ dự án chọn: hình nền tĩnh không có "bây giờ"); hộp nhắc mặc định = giờ hạn; JSON `version` giữ 1; bộ sửa hạn đóng bằng nút "Xong" (không blur). 4 phiếu T-9.1 → T-9.2 → T-9.3 → T-9.END, đều đủ điều kiện Gemini; 2 S4 của M8 gộp T-9.3. Làm ngay, không chờ G1–G5; deploy M8 + M9 gộp một lần push.
...
- Toast đè Hoàn tác (M6 a): gộp — thao tác có Hoàn tác liên tiếp trong lúc toast còn hiện thì một nút Hoàn tác khôi phục tất cả (mới nhất trước). Lý do: không mất thao tác, không cần hàng đợi toast/chữ mới.
  - Siết (B-016 vòng 1 REGRESSION `m6-todo-gestures.spec.ts:172`): chỉ gộp khi giữa hai lần `showToast` có hành động, store nhận ĐÚNG 1 `dispatch` (chính thao tác mới) — cài bằng SO THAM CHIẾU STATE (state vẽ gần nhất === state lưu sau thao tác trước), KHÔNG dùng `store.subscribe` (test khóa `store-subscribe-guard`; SC-005). Có thay đổi khác xen giữa (thêm/sửa/kéo việc…) → xóa danh sách chờ, chỉ giữ Hoàn tác mới nhất. Lý do: Hoàn tác không được khôi phục thứ người dùng đã "bỏ qua" bằng thao tác khác.
