...
- Đang làm: CHỜ CHỦ DỰ ÁN deploy lần đầu (D-019/D-020): nhánh cục bộ đã đổi `master`→`main`, remote `origin` = `https://github.com/thaihuy1012/LichKhoa.git`, đã merge `-s ours` lịch sử 5 commit tải tay (cây = mã nguồn) → push thường, không force. `.claude/settings.json` cấm Quản lý `git push` → Chủ dự án: (1) GitHub Settings → Pages → Source = GitHub Actions; (2) `! git push -u origin main`; (3) tab Actions xanh → mở `https://thaihuy1012.github.io/LichKhoa/` thử T-4.10 (ghim ghi chú). Sau đó → Kiến trúc sư duyệt M4 → nghiệm thu cuối.
