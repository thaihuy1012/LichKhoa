...
## Đã đóng gần đây
- **SC-005** (S2, ĐÃ ĐÓNG 2026-09-22) — B-016 gộp Hoàn tác: regression m6:172 (vòng 2 thực chất test trên `dist/` cũ)
...
## SC-005 — B-016 gộp Hoàn tác gây REGRESSION test khóa M6 · Mức S2 · Trạng thái: ĐÃ ĐÓNG
...
- ĐÓNG 2026-09-22: nguyên nhân gốc (1) vòng 1 là lỗi thật (gộp vô điều kiện); vòng 2 logic đúng nhưng Gemini chạy playwright không build lại → e2e (vite preview phục vụ `dist/`) test bản v1 cũ; (2) v2 dùng `store.subscribe` trong Events.tsx vi phạm test khóa `store-subscribe-guard.test.ts:45` — lỗi do phiếu của Quản lý. Sửa: so tham chiếu state (state vẽ gần nhất trùng state lưu sau thao tác có Hoàn tác trước → mới gộp). Kiểm đột biến (ép gộp vô điều kiện → test X→Y→Z fail). kiem-thu độc lập PASS (3 file ×3 = 66/0; check 314 unit ×2 TZ, 112 e2e / 14 skip; `SC-005-kiem*.log`). Commit: xem git log "B-016".
